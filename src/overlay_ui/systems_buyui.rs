...
use crate::{
    ...
    resourcey::{CurrentCartBlock, KeyboardTarget, TargetType, TileCartVec},
    statey::ExploreState,
...
use super::layout_buy_menu::{ButtonBack, ButtonGo};
...
pub fn leftright_cart_button_system(
    ...
    mut param_set: ParamSet<(
        ...
        Query<&mut Text, With<CurrentBlockOwnerText>>,
            ...
            Interaction::Pressed => {
                ...
                *color = PRESSED_BUTTON.into();
                ...
                for mut text in param_set.p1().iter_mut() {
                    text.sections[0].value = format!("Value: {}", cart.vec[cart.index].value);
                }
                ...
                for mut text in param_set.p3().iter_mut() {
                    text.sections[0].value = format!("Owner: {}", cart.vec[cart.index].owner);
                }
...
pub fn leftright_cart_button_system_set_new_text(
    ...
    for (interaction, mut color, button_comp) in &mut interaction_query {
        let indexee = cart.index;
        info!("indexee {}", indexee);
...
pub fn new_color_button_system(
    mut interaction_query: Query<
        (&Interaction, &mut BackgroundColor),
        (Changed<Interaction>, With<NewBlockColorButton>),
    >,
    //mut text_query: Query<&mut Text, With<NewBlockLnAddressText>>,
    //keyboard_text: Res<KeyboardData>,
    mut target: ResMut<KeyboardTarget>,
...
pub fn new_message_button_system(
    mut interaction_query: Query<
        (&Interaction, &mut BackgroundColor),
        (Changed<Interaction>, With<NewBlockMessageButton>),
    >,
    //mut text_query: Query<&mut Text, With<NewBlockLnAddressText>>,
    //keyboard_text: Res<KeyboardData>,
    mut target: ResMut<KeyboardTarget>,
